...
#ifndef SPARSE_TABLE_2D_MIN
// should be 1-base
class sparse_table_2d_min {
	private:
	...
	public:
		...
		sparse_table_2d_min(int n, int m) {
			this->n = n, this->m = m;
			lgn = 1;
				...
				for (int c=m; c>=1; c--) {
					...
					for (int br=0; br<lgn; br++) {
						...
						for (int bc=0; bc<lgn; bc++) {
							...
							table[r][c][br][bc] =
								min(table[r][c][pbr][pbc],
								  min(table[r][nc][pbr][pbc],
								    min(table[nr][c][pbr][pbc],
										table[nr][nc][pbr][pbc])));
						}
					}
				}
		...
		dtype min_value(int rs, int cs, int rf, int cf) {
			...
			return
				min(table[rs][cs][br][bc],
				  min(table[rf-rcv][cs][br][bc],
				    min(table[rs][cf-ccv][br][bc],
						table[rf-rcv][cf-ccv][br][bc])));
		}
};
#endif

#define SPARSE_TABLE_2D_MIN
//////// ---- Woosung Song's Source Code ---- ////////

sparse_table_2d_min spt(10, 10);
...
int main() {
	...
	while (1) {
		...
		int rs, cs, rf, cf;
		cin >> rs >> cs >> rf >> cf;
		cout << spt.min_value(rs, cs, rf, cf) << endl;
	}
}
