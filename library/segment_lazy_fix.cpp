...
#ifndef SEGMENT_LAZY_FIX_DEFINED
template <class dtype, class ltype>
class segment_lazy_fix {
	private:
		dtype SGID = 0; // SUM
		ltype LZID = 0; // FIX: No identity
	...
	public:
		...
		segment_lazy_fix<dtype, ltype>() {}
		segment_lazy_fix<dtype, ltype>(int min_index, int max_index) {
...
#define SEGMENT_LAZY_FIX_DEFINED
//////// ---- Woosung Song's Source Code ---- ////////
...
int main() {
	...
	scanf("%d%d%d", &n,&m,&k);
	m += k;
	segment_lazy_fix<ll,ll> seg(1, n);
	for (int i=1; i<=n; i++) {
		ll a; scanf("%lld", &a);
