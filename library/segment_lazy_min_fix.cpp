...
#ifndef SEGMENT_LAZY_MIN_FIX_DEFINED
template <class dtype, class ltype>
class segment_tree_min_fix {
	private:
		dtype SGID = numeric_limits<dtype>::max(); // MIN
	...
	public:
		...
		segment_tree_min_fix<dtype, ltype>() {}
		segment_tree_min_fix<dtype, ltype>(int min_index, int max_index) {
...
#define SEGMENT_LAZY_MIN_FIX_DEFINED
//////// ---- Woosung Song's Source Code ---- ////////
...
int main() {
	...
	scanf("%d%d%d", &n,&m,&k);
	m += k;
	segment_tree_min_fix<ll,ll> seg(1, n);
	for (int i=1; i<=n; i++) {
		ll a; scanf("%lld", &a);
