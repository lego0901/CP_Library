...
#ifndef SEGMENT_LAZY_MAX_ADD_DEFINED
template <class dtype, class ltype>
class segment_lazy_max_add {
	private:
		dtype SGINIT = 0; // SEGMENT INIT
		dtype SGID = numeric_limits<dtype>::min()/2; // MAX
		ltype LZID = 0; // ADD
	...
	public:
		...
		segment_lazy_max_add<dtype, ltype>() {}
		segment_lazy_max_add<dtype, ltype>(int min_index, int max_index) {
...
#define SEGMENT_LAZY_MAX_ADD_DEFINED
//////// ---- Woosung Song's Source Code ---- ////////
...
int main() {
	...
	scanf("%d%d%d", &n,&m,&k);
	m += k;
	segment_lazy_max_add<ll,ll> seg(1, n);
	for (int i=1; i<=n; i++) {
		ll a; scanf("%lld", &a);
