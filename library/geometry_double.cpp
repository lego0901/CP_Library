...
namespace geometry_double {
	...
	const dtype GEOMETRY_EPSILON = 1e-6;
	const double GEOMETRY_PI = 2.0 * acos(0.0);

	bool is_zero(dtype x) { return (x < GEOMETRY_EPSILON) && (x > -GEOMETRY_EPSILON); }
	bool is_nzero(dtype x) { return !is_zero(x); }

	int sign(dtype x) { return (x>-GEOMETRY_EPSILON) - (x<GEOMETRY_EPSILON); }

	class point {
		public:
			dtype x, y;
			point(dtype x_=0, dtype y_=0) : x(x_), y(y_) {}
			bool operator==(const point &o) const { return is_zero(x-o.x) && is_zero(y-o.y); }
			bool operator<(const point &o) const { return is_zero(x-o.x)? y<o.y : x<o.x; }
			bool operator<=(const point &o) const { return *this<o || *this==o; }
			bool operator>(const point &o) const { return o<*this; }
			bool operator>=(const point &o) const { return o<=*this; }
			point operator+(const point &o) const { return point(x+o.x, y+o.y); }
			point operator-(const point &o) const { return point(x-o.x, y-o.y); }
			point operator*(const dtype k) const { return point(x*k, y*k); }
			point operator/(const dtype k) const { return point(x/k, y/k); }
			dtype dist2() const { return x*x + y*y; }
			dtype dist2(const point &o) const { return (*this-o).dist2(); }
			dtype dist() const { return sqrt(dist2()); }
			dtype dist(const point &o) const { return (*this-o).dist(); }
			dtype dot(const point &o) const { return x*o.x + y*o.y; }
			dtype dot(const point &a, const point &b) const { return (a-*this).dot(b-*this); }
			dtype cross(const point &o) const { return x*o.y - y*o.x; }
			dtype cross(const point &a, const point &b) const { return (a-*this).cross(b-*this); }
	...
	class line {
		public:
			...
			dtype length2() { return a.dist2(b); }
			dtype length() { return a.dist(b); }
			bool contains(const point &o) const { return o.ccw(a, b)? (min(a, b) <= o && o <= max(a, b)) : 0; }
			dtype det(const line &o) const { return (b-a).cross(o.b-o.a); }
			bool parallel(const line &o) const { return is_zero(det(o)); }
			...
			int num_intersection(const line &o) const {
				if (parallel(o)) {
					if (is_nzero(a.ccw(b, o.a)) || max(a,b)<min(o.a,o.b) || max(o.a,o.b)<min(a,b)) return 0;
	...
	pair<point,point> __brute_force_farthest_pair(vector<point> &cvh) {
		int c = (int) cvh.size();
		dtype record_dist2 = 0;
		int record_i = 0, record_j = 0;
		for (int i=0; i<c; i++)
			for (int j=i+1; j<c; j++) {
				dtype d2 = cvh[i].dist2(cvh[j]);
				if (d2 > record_dist2)
	...
	pair<point,point> __brute_force_closest_pair(vector<point> &points) {
		int n = (int) points.size();
		dtype record_dist2 = numeric_limits<ll>::max();
		int record_i = 0, record_j = 0;
		for (int i=0; i<n; i++)
			for (int j=i+1; j<n; j++) {
				dtype d2 = points[i].dist2(points[j]);
				if (d2 < record_dist2)
					record_dist2 = d2, record_i = i, record_j = j;
			}
	...
	pair<point,point> closest_pair(vector<point> &points, bool brute_force=0) {
		...
		point l1, l2, r1, r2, p1, p2; dtype record_dist2 = 0;
		tie(l1, l2) = closest_pair(left); dtype ld2 = l1.dist2(l2);
		tie(r1, r2) = closest_pair(right); dtype rd2 = r1.dist2(r2);
		if (ld2 < rd2) record_dist2 = ld2, p1 = l1, p2 = l2;
		else record_dist2 = rd2, p1 = r1, p2 = r2;

		dtype x_mid = (left[(int) left.size()-1].x + right[0].x) / 2;
...
int main() {
	using namespace geometry_double;
	int n;
	cin >> n;
	vector<point> points;
	for (int i=0; i<n; i++) {
