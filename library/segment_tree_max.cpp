...
#ifndef SEGMENT_TREE_MAX_DEFINED
template <class dtype>
class segment_tree_max {
	private:
		dtype SGID = numeric_limits<dtype>::min();

		dtype update(int tn, int l, int r, int p, dtype v) {
			if (l <= p && p <= r) {
				if (l < r) {
					int m = (l + r) / 2;
					tree[tn] = max(update(tn*2, l, m, p, v),update(tn*2+1, m+1, r, p, v));
				} else tree[tn] = v;
			}
			return tree[tn];
		}
	...
	public:
		...
		segment_tree_max<dtype>() {}
		segment_tree_max<dtype>(int min_index, int max_index) {
...
#define SEGMENT_TREE_MAX_DEFINED
//////// ---- Woosung Song's Source Code ---- ////////
