...
#ifndef SEGMENT_LAZY_ADD_DEFINED
template <class dtype, class ltype>
class segment_lazy_add {
	private:
		dtype SGID = 0; // SUM
		ltype LZID = 0; // ADD

		inline dtype value(int tn, int l, int r) {
			return prop[tn]? tree[tn] + lazy[tn]*(r-l+1) : tree[tn];
		}
		...
		dtype update_interval(int tn, int l, int r, int ql, int qr, ltype lz) {
			if (ql <= l && r <= qr) {
				...
				propagate(tn, l, r);
				int m = (l + r) / 2;
				tree[tn] = update_interval(tn*2, l, m, ql, qr, lz)
	...
	public:
		...
		segment_lazy_add<dtype, ltype>() {}
		segment_lazy_add<dtype, ltype>(int min_index, int max_index) {
...
#define SEGMENT_LAZY_ADD_DEFINED
//////// ---- Woosung Song's Source Code ---- ////////
...
int main() {
	...
	scanf("%d%d%d", &n,&m,&k);
	m += k;
	segment_lazy_add<ll,ll> seg(1, n);
	for (int i=1; i<=n; i++) {
		ll a; scanf("%lld", &a);
