...
#ifndef SPLAY_TREE_LAZY_DEFINED
typedef long long ll;
typedef ll lazy_type;
...
struct __node {
	...
	int key, cnt;
	sum_type sum, value;
	lazy_type lazy; // 필요에 따라 알아서 정해주면 됨

	__node(int k=0, sum_type v=0) : l(0), r(0), p(0), key(k), cnt(1), sum(v), value(v), lazy(0) {}
};

template <class sum_type>
class splay_tree_lazy {
	private:
		...
		void lazy(node *x) { 
			x->value += x->lazy;
			if (x->l) x->l->lazy += x->lazy, x->l->sum += x->l->cnt * x->lazy;
			if (x->r) x->r->lazy += x->lazy, x->r->sum += x->r->cnt * x->lazy;
			x->lazy = 0;
		}


	public:
		splay_tree_lazy(sum_op add_op=0, sum_type add_id=0) : root(NULL), __cnt(0) {
	...
	private:
		...
		void kth(int k) {
			node *x = root;
			while (1) {
				while (x->l && x->l->cnt > k) x = x->l, lazy(x);
				if (x->l) k -= x->l->cnt;
				if (k-- == 0) break;
				x = x->r, lazy(x);
			}
	...
	public:
		...
		void refresh_interval(int l, int r, lazy_type v) {
			node *x = interval(l, r);
			x->lazy += v;
			x->sum += x->cnt * v;
		}
...
#define SPLAY_TREE_LAZY_DEFINED
//////// ---- Woosung Song's Source Code ---- ////////

int main() {
	typedef long long ll;
	splay_tree_lazy<ll> sp;
